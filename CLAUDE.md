...
- Os arquivos JS/CSS são referenciados no `index.html` com um query string de versão (`?v=8`) para cache-busting; ao alterar `words.js`, `audio.js`, `game.js` ou `style.css`, incremente esse número em todas as tags `<script>`/`<link>` no `index.html`.
