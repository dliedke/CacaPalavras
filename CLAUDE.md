...
3. **`js/game.js`** — IIFE principal com toda a lógica de jogo, dividida em seções internas (ver comentários de bloco no arquivo):
   ...
   - **Estado** — um único objeto `estado` (não reativo) guarda dificuldade/categoria atuais, a grade, as soluções (`solucoes`, com `{palavra, exibicao, celulas, achado, cor}`), pontuação, sequência e o timer. Toda mudança de estado é seguida por chamadas manuais de re-render (`renderGrade`, `renderLista`, `atualizarPlacar`).
...
- Os arquivos JS/CSS são referenciados no `index.html` com um query string de versão (`?v=6`) para cache-busting; ao alterar `words.js`, `audio.js`, `game.js` ou `style.css`, incremente esse número em todas as tags `<script>`/`<link>` no `index.html`.
